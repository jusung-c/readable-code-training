...
- 모든 셀에 깃발을 다 꽂은 경우는 승리 조건이 아닌데도 지뢰를 모두 찾아 승리한 것으로 처리되고 있다.
- 승리 조건을 다시 생각해보자.
  - 빈 셀 or 숫자 셀: 오픈되면 오케이입니다~
  - 지뢰 셀: 깃발이 꽂혀야 오케이입니다.
- 객체 지향적으로 책임 단위로 쪼개놨기 때문에 Cell의 isChecked() 메서드만 살짝 수정해줘도 버그를 수정할 수 있었다.
...
- 게임판의 크기를 엄청 크게 설정하고 게임을 진행하면 StackOverflowError가 발생한다.
- 기존 로직은 재귀를 활용한 DFS(깊이 우선 탐색) 알고리즘을 활용한 방식이었다.
  - 스레드마다 생기는 스택 영역에는 함수를 호출할 때마다 frame이 쌓이므로 스택 영역을 초과할 경우 StackOverflowError가 발생한다.
- 각 Cell의 모든 메서드 정보가 필요한 것이 아니다. 각 Cell의 CellPosition만 있으면 기존의 모든 로직을 할 수 있기 때문에 Stack을 활용해서 DFS를 구현해보자
  - 기존에는 스택 메모리 영역에 많은 프레임이 쌓여서 에러가 생겼는데, 이 방법을 쓰면 직접 CellPosition만을 가지는 Stack을 사용했기 때문에 에러가 발생하지 않는다.
